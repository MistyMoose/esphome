...
/*
 * Most of the code in this integration is based on the vl6180x library
 * by Pololu (Pololu Corporation), which in turn is based on the vl6180x
 * API from ST.
 *
 * For more information about licensing, please view the included LICENSE.txt file
 * in the vl6180x integration directory.
 */
...
std::list<vl6180xSensor *> vl6180xSensor::vl53_sensors;  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
bool vl6180xSensor::enable_pin_setup_complete = false;   // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)

vl6180xSensor::vl6180xSensor() { vl6180xSensor::vl53_sensors.push_back(this); }

void vl6180xSensor::dump_config() {
  LOG_SENSOR("", "vl6180x", this);
...
void vl6180xSensor::setup() {
  ...
  if (!esphome::vl6180x::vl6180xSensor::enable_pin_setup_complete) {
    ...
    esphome::vl6180x::vl6180xSensor::enable_pin_setup_complete = true;
  }
  ...
  // The following is different for vl6180x vs VL53L1X
  // I2C_SXXXX_DEVICE_ADDRESS = 0x8A for vl6180x
...
uint32_t vl6180xSensor::get_measurement_timing_budget_() {
...
bool vl6180xSensor::set_measurement_timing_budget_(uint32_t budget_us) {
  SequenceStepEnables enables{};
  SequenceStepTimeouts timeouts{};
  ...
  if (enables.final_range) {
    ...
    // (SequenceStepId == vl6180x_SEQUENCESTEP_FINAL_RANGE)
...
void vl6180xSensor::get_sequence_step_enables_(SequenceStepEnables *enables) {
...
void vl6180xSensor::get_sequence_step_timeouts_(SequenceStepEnables const *enables, SequenceStepTimeouts *timeouts) {
...
uint8_t vl6180xSensor::get_vcsel_pulse_period_(VcselPeriodType type) {
  uint8_t vcsel;
...
uint32_t vl6180xSensor::get_macro_period_(uint8_t vcsel_period_pclks) {
  return ((2304UL * vcsel_period_pclks * 1655UL) + 500UL) / 1000UL;
}

uint32_t vl6180xSensor::timeout_mclks_to_microseconds_(uint16_t timeout_period_mclks, uint8_t vcsel_period_pclks) {
  uint32_t macro_period_ns = get_macro_period_(vcsel_period_pclks);
  return ((timeout_period_mclks * macro_period_ns) + (macro_period_ns / 2)) / 1000;
}

uint32_t vl6180xSensor::timeout_microseconds_to_mclks_(uint32_t timeout_period_us, uint8_t vcsel_period_pclks) {
  uint32_t macro_period_ns = get_macro_period_(vcsel_period_pclks);
  return (((timeout_period_us * 1000) + (macro_period_ns / 2)) / macro_period_ns);
}

uint16_t vl6180xSensor::decode_timeout_(uint16_t reg_val) {
...
uint16_t vl6180xSensor::encode_timeout_(uint16_t timeout_mclks) {
  // format: "(LSByte * 2^MSByte) + 1"
  uint32_t ls_byte = 0;
  uint16_t ms_byte = 0;
...
bool vl6180xSensor::perform_single_ref_calibration_(uint8_t vhv_init_byte) {
  reg(0x00) = 0x01 | vhv_init_byte;  // vl6180x_REG_SYSRANGE_MODE_START_STOP
